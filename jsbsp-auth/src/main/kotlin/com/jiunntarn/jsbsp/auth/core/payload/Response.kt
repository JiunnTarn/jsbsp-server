package com.jiunntarn.jsbsp.auth.core.payload
...
data class Response(var code: Int, var data: Any? = null, var message: String? = null, var accessToken: String? = null) {
    companion object {
        fun ok(data: Any? = null, message: String? = null, token: String? = null): Response {
            return Response(code = 0, data = data, message = message, accessToken = token)
        }
    ...
    fun toJson(): String = GsonBuilder().setFieldNamingPolicy(FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES).create().toJson(this)
}